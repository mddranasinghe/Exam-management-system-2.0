...
<body>
    ...
    <section>
                            ...
                            <div class="form-group row">
                                <form name="Registration" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                                    ...
                                    if (isset($_POST['submit'])) {
                                        ...
                                        $filenameResit = $_FILES['myfile_resit']['name'][$indexNo];
                                        $tempnameResit = $_FILES['myfile_resit']['tmp_name'];
        ...
        '$_POST[Bst_attempt_2]',
        '$_POST[Cst_attempt_2]');");
                                            $stmt = $conn->prepare("SELECT * FROM  resitTracking WHERE Registration_No='$_POST[Registration_No]' AND Exam='$_POST[Name_of_the_examination]'");
                                            $stmt->execute();
                                            $results = $stmt->get_result();
                                            if ($results->num_rows > 0) {
                                                //update the application submition date and time on a resubmission
                                                mysqli_query($conn, "UPDATE resitTracking SET 'Date'=$date,'time'=$time'");
                                            } else {
                                    ...
                                    <?php
                                        ...
                                        if (move_uploaded_file($tempnameResit, $folder)) {
